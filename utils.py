...
def process_sampling_numbers(num_neighbors, num_layers):
    if not type(num_neighbors)==list: 
...
class RandHyperEdgeSampler(object):
    ...
    def sample(self, src_l, he_offset_l, ts):
        neg_data = {}
        ...
        for he_idx in range(len(he_offset_l)-1):
            ...
            kept_nodes = set(np.random.choice(he_nodes, n_kept_nodes))
            random_nodes = set(np.random.choice(remained_nodes, n_random_nodes))
            kept_nodes |= random_nodes
            neg_data[i] = (kept_nodes, ts[i])
            i = i+1
        return neg_data

def construct_algo_data_given_he_ids(pos_data):
    src_l, he_offset_l = [], [0]
    prev_he_offset_val = 0
    for edge in pos_data.values():
        he_nodes = edge[0]
        src_l.extend(list(he_nodes))
        prev_he_offset_val += len(he_nodes)
        he_offset_l.append(prev_he_offset_val)
    
    return src_l, he_offset_l
...
def get_args():
    ...
    parser.add_argument('--seed', type=int, default=1234, help='random seed for all randomized algorithms')
    ...
    parser.add_argument('--node_out_channels', type=int, default=128, help='')
    parser.add_argument('--time_channels', type=int, default=64, help='')
    parser.add_argument('--hyperEdge_hidden_channels', type=int, default=128, help='')
    parser.add_argument('--decoder_hid_channels', type=int, default=128, help='')

    # encoding tree parameters
    parser.add_argument('--hypergraph_tree_height', type=int, default=7, help='')
    parser.add_argument('--hypergraph_big', type=bool, default=False, help='')
    parser.add_argument('--hypergraph_big_sample', type=int, default=2, help='')
    parser.add_argument('--linegraph_tree_height', type=int, default=20, help='')
    parser.add_argument('--linegraph_big', type=bool, default=True, help='')
    parser.add_argument('--linegraph_big_sample', type=int, default=2, help='')
    parser.add_argument('--K_for_graph_compression', type=int, default=3, help='')
    parser.add_argument('--max_early_stop', type=int, default=100, help='')
    ...
    parser.add_argument('--pre_epochs', type=int, default=200, help='number of epochs')
